 ...
 */
	...
	public static void main(String args[]){
		try  {
			Class.forName("chapter9.Constants");
		} catch (Exception e)  {
			e.printStackTrace();
			System.exit(3);
		}
...
//		}
		...
		MyThread threadA;
		//将任务进行分割
		NetC client = new NetC();  //初始化任务 给矩阵赋初始值
		...
		//初始化线程A，寻找服务器，进行计算
		threadA = new MyThread(arrA, arrC, sServer1, sPort1,1);
		threadA.start();
		//初始化线程B，寻找服务器，进行计算
		//threadB = new MyThread(arrB, arrC, sServer2, sPort2,2);
		//threadB.start();
		//++等待任务结束
		...
		//等待任务结束，取得计算结果。
		while(bSuccessful && arrResult1 == null){
//			try{
//				//休眠200毫秒，等待计算任务结束
//				Thread.sleep(20);
//				
...
//			}
		...
		if(bSuccessful){
			...
			//System.out.println(Integer.parseInt(time1) - Integer.parseInt(time));
...
class MyThread extends Thread{
	...
	public MyThread(int[][] arrA, int[][] arrB, String sName, String sPort, int number){
		m_sServerName = sName;
		m_sServerPort = sPort;
